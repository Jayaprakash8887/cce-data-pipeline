...
| `schema/01`–`schema/06` `*.sql` | ClickHouse | applied once (in order) | Base tables → Kafka-engine queues + consumer MVs → aggregation MVs → indexes → dictionaries → current-state rollups |
| `infra/clickhouse/*.xml` | ClickHouse | mounted (config.d / users.d) | `config.xml` (server tuning, metrics), `named-collections.xml` (`cce_kafka` broker via `from_env`), `users.xml` (`cce_pipeline` read-only + `final=1`) |
